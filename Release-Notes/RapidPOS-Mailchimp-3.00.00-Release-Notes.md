...
_Adds text message (SMS) customer support, more configuration for downloading changes from Mailchimp, and a new toolbar button for triggering a sync on demand, plus several reliability fixes._
...
- Downloading from Mailchimp and uploading to Mailchimp now run on separate schedules. Uploads stay frequent (every minute by default); downloads run less often by default (midnight, 8am, and 4pm), and this schedule is configurable.
...
### Run a sync on demand from the Customers screen

A new toolbar button on the Counterpoint Customers screen lets you trigger an out-of-cycle sync immediately instead of waiting for the next scheduled run. It uses the same on-demand sync mechanism as the existing config-screen checkbox — this just adds a more convenient way to trigger it, not a separate method.
